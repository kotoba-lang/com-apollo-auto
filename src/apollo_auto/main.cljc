(ns apollo-auto.main
  "Kotodama WASM entrypoint for the Apollo_auto clean-room actor (L4) — Clojure port.
  ...
  facts (`apollo_auto.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "apollo_auto")
(def tier "L4")
...
(def entity-specs
  [{:entity "Chassis"             :plural "chassises"           :id-prefix "apolloau_cha"
    :fields [:engineStarted :speedMps :throttlePercentage :brakePercentage :steeringPercentage :parkingBrake :wiper :drivingMode :errorCode :gearLocation :frontBumperEvent]
    :required [:engineStarted :speedMps]
    :coerce {:engineStarted :bool :speedMps :float :throttlePercentage :float :brakePercentage :float :steeringPercentage :float :parkingBrake :bool :wiper :bool}
    :refs {}}
   {:entity "PerceptionObstacle"  :plural "perceptionobstacles" :id-prefix "apolloau_per"
    :fields [:theta :length :width :height :trackingTime :timestamp :confidence :type :subType]
    :required [:theta :length]
    :coerce {:theta :float :length :float :width :float :height :float :trackingTime :float :timestamp :float :confidence :float}
    :refs {}}
   {:entity "ADCTrajectory"       :plural "adctrajectories"     :id-prefix "apolloau_adc"
    :fields [:totalPathLength :totalPathTime :isReplan :replanReason :carInDeadEnd :isCollision :rightOfWayStatus :trajectoryType]
    :required [:totalPathLength :totalPathTime]
    :coerce {:totalPathLength :float :totalPathTime :float :isReplan :bool :carInDeadEnd :bool :isCollision :bool}
    :refs {}}
   {:entity "LocalizationStatus"  :plural "localizationstatuses" :id-prefix "apolloau_loc"
    :fields [:measurementTime :stateMessage :fusionStatus :gnssStatus :lidarStatus]
    :required [:measurementTime :stateMessage]
    :coerce {:measurementTime :float}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"apollo_auto.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "apollo_auto-compat" :tier tier :entities entities} 200])
...
(defn start! [] :apollo_auto-compat/ready)
